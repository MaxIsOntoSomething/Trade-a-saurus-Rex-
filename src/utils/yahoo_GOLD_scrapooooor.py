...
class YahooGoldScraper:
    """Scrapes Gold historical data from Yahoo Finance"""
    ...
    async def get_gold_data(self, days: int = 90) -> Dict[str, float]:
        """
        Fetch Gold historical data from Yahoo Finance
        ...
        try:
            ...
            # Yahoo Finance API URL for Gold Futures (GC=F)
            ...
            headers = {
                ...
                "Referer": "https://finance.yahoo.com/quote/GC=F",
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, params=params, headers=headers) as response:
                    if response.status == 200:
                        ...
                        if result:
                            logger.info(f"Successfully retrieved {len(result)} days of Gold data from Yahoo Finance")
                            return result
                    ...
                    if response.status == 406:
                        ...
                        alt_url = f"https://query2.finance.yahoo.com/v8/finance/chart/GC=F"
                        ...
                        async with session.get(alt_url, params=params, headers=headers) as alt_response:
                            if alt_response.status == 200:
                                ...
                                if result:
                                    logger.info(f"Successfully retrieved {len(result)} days of Gold data from Yahoo Finance (alternative endpoint)")
                                    return result
                            else:
            ...
            logger.error("All attempts to retrieve Gold data failed")
            return {}
                    
        except Exception as e:
            logger.error(f"Error fetching Gold data from Yahoo Finance: {e}", exc_info=True)
            return {}
    ...
    async def get_gold_daily_change(self) -> Optional[float]:
        """Get the current day's Gold percentage change"""
        try:
            await self._respect_rate_limit()
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params, headers=headers) as response:
                    if response.status != 200:
                        logger.error(f"Failed to get Gold daily change: {response.status}")
                        return None
            ...
            logger.error(f"Error getting Gold daily change: {e}")
            return None

    async def get_ytd_data(self) -> Dict[str, float]:
        """
        Get year-to-date performance data for Gold
        ...
        try:
            ...
            data = await self.get_gold_data(days_since_start + 10)  # Add buffer for holidays/weekends
            
            if not data:
                logger.error("Failed to get Gold data for YTD calculation")
                return {}
            ...
            logger.info(f"Generated YTD data for Gold with {len(ytd_data)} data points")
            return ytd_data
            
        except Exception as e:
            logger.error(f"Error getting YTD Gold data: {e}", exc_info=True)
            return {} 